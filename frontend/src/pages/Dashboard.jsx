...
import WorkflowPipelineHub from '../components/WorkflowPipelineHub';
...
export default function Dashboard() {
  ...
  const handleRunAnalysis = async () => {
    setAnalyzing(true);
    addToast('Executing ML & Multi-Signal Analytical Cycle across records...', 'info');
    try {
      await api.runAnalysis();
      addToast('Analytical Cycle completed successfully. Risk scores updated.', 'success');
      loadData();
    } catch (e) {
      addToast('Analysis run failed: ' + e.message, 'error');
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* High Risk Priority */}
        <div className="bg-white p-5 rounded-2xl border border-l-4 border-l-red-600 border-slate-200 shadow-xs space-y-1">
        ...
        {/* Active Inquiries */}
        <div className="bg-white p-5 rounded-2xl border border-l-4 border-l-blue-600 border-slate-200 shadow-xs space-y-1">
      ...
      {/* VIEW 1: GUIDED STEP-BY-STEP OPERATIONAL FLOW */}
      {viewMode === 'pipeline' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          {/* Flagship Case Inquest Spotlight Card */}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
              ...
              <div className="space-y-2 max-w-3xl">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-red-100 text-red-800 border border-red-200 font-mono">
                    CASE SPOTLIGHT • #{flagshipCase.workId}
                  </span>
                ...
                <p className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200 leading-relaxed">
                  <strong>Administrative Anomaly Finding:</strong> Sanctioned at <strong>{formatINR(flagshipCase.cost || 4800000)}</strong> compared to sector peer median <strong>₹31.20 Lakh</strong> (+53.8% outlier) • Near-identical proposal detected 420m away • Statutory completion certificate is missing on file.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <Link
                  ...
                  className="px-5 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition flex items-center gap-2 shadow-md hover:shadow-lg transform active:scale-95"
                >
                  <Eye className="w-4 h-4 text-blue-400" />
                  <span>Inspect Risk Passport Dossier</span>
      ...
      {/* VIEW 2: DEEP ANALYTICAL CHARTS & SCRUTINY REGISTER */}
      {viewMode === 'analytics' && (
        <div className="space-y-6 animate-in fade-in duration-200">
